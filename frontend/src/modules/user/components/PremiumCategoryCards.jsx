        ...
        >
            <div className="container mx-auto px-4 max-w-[1100px]">
                ...
                <div className="flex flex-col items-center mb-6 md:mb-20 relative z-50">
                    <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-[#C59B27] mb-1.5 md:mb-3">Gift the Excellence</span>
                    <h2 className="text-2xl md:text-4xl font-serif text-stone-900 tracking-tight leading-none text-center">
                        Shop by <span className="italic font-light text-[#C59B27]">Recipient</span>
                    </h2>
                    <div className="w-8 h-[1px] md:w-10 md:h-[2px] bg-[#C59B27]/60 mt-3 md:mt-5" />
                </div>
                ...
                <div className="flex flex-col gap-8 md:grid md:grid-cols-2 md:gap-16 relative">
                    {RECIPIENTS.map((item, idx) => (
                        <motion.div
                            ...
                            className="relative h-[115px] md:h-[180px] bg-[#FAF8F5] rounded-[24px] md:rounded-[48px] cursor-pointer group shadow-sm flex items-center border border-[#E8DFD0]"
                        >
                            {/* Obsidian Inset Box */}
                            <div
                                className={`absolute top-[6%] bottom-[6%] w-[75%] bg-gradient-to-br from-[#1C1917] via-[#141211] to-[#0A0908] border border-[#C59B27]/30 rounded-[20px] md:rounded-[44px] flex items-center justify-center p-3 md:p-8 z-10 transition-all duration-500 group-hover:shadow-[0_15px_30px_rgba(197,155,39,0.18)] 
                                ${item.layout === 'left' ? 'left-[2%]' : 'right-[2%]'}`}
                            >
                                {/* Shop Now Button inside obsidian box */}
                                <div className={`flex items-center gap-3 md:gap-4 ${item.layout === 'right' ? 'flex-row-reverse translate-x-[25px] md:translate-x-[50px]' : '-translate-x-[25px] md:-translate-x-[50px]'}`}>
                                    <span className="text-[#FAF8F5] text-[15px] md:text-[22px] font-semibold tracking-[0.1em] font-sans drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)] group-hover:tracking-[0.15em] transition-all duration-700">
                                        {item.title}
                                    </span>
                                    <div className="w-6 h-6 md:w-9 md:h-9 rounded-full bg-white flex items-center justify-center text-[#141211] group-hover:bg-[#C59B27] group-hover:text-[#141211] transition-all duration-500 shadow-lg border border-white/10">
                                        {item.layout === 'left' ? (
