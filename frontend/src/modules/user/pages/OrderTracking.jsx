        ...
        <div className="min-h-screen bg-white font-sans pt-0 md:pt-12 pb-12 selection:bg-[#C59B27] selection:text-[#141211]">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-0">
                ...
                <div className="space-y-8">
                    ...
                    {returnRequest && (
                        <div className="bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden group hover:shadow-md transition-shadow relative z-10">
                                    ...
                                    {returnRequest.items.map((item, idx) => (
                                        <div key={idx} className="relative">
                                            <ProductThumb
                                                src={item.image}
                                                alt={item.name}
                                                className="w-12 h-12 md:w-14 md:h-14 rounded-lg object-cover border border-stone-200 grayscale-[0.2]"
                                                fallbackClassName="border-stone-200"
                    ...
                    <div className="bg-white border border-stone-200 rounded-xl shadow-sm overflow-hidden group hover:shadow-md transition-shadow relative">
                        {/* Connecting line to show flow (Upwards to Return if exists) */}
                        {returnRequest && <div className="absolute top-[-24px] left-8 w-0.5 h-6 bg-stone-200 z-0"></div>}
                                ...
                                {order.items.map((item, idx) => (
                                    <div key={idx} className="flex-shrink-0 w-12 md:w-14">
                                        <ProductThumb
                                            src={item.image}
                                            alt={item.name}
                                            className="w-12 h-12 md:w-14 md:h-14 rounded-lg object-cover border border-stone-200"
                                            fallbackClassName="border-stone-200"
                                            fallbackIconClassName="w-4 h-4"
                                        />
                                    </div>
