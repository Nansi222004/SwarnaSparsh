...
const AdminScanner = () => {
  ...
  const playSuccessEffects = () => {
    try {
      ...
      // Ignore sound/vibration failures
    }
  };
  ...
  const handleProductScan = async (code) => {
    setLoading(true);
    try {
      const response = await adminDirectSaleService.preview({
        serialCode: code,
      });
      if (!response?.success) {
        throw new Error(response?.message || "Failed to preview barcode");
      }
      ...
      toast.success(
        data.available
          ? "Item verified: Ready for direct sale"
          : "Item already sold or not available"
      );
    } catch (err) {
      toast.error(
        err.response?.data?.message || err.message || "Invalid or unassigned barcode"
      );
      setScanResult(null);
    } finally {
      setLoading(false);
  ...
  const confirmDirectSale = async () => {
    ...
    setConfirming(true);
    try {
      const res = await adminDirectSaleService.confirm({
        serialCode: scannedProduct.serialCode,
        paymentMethod: "cash",
      });
      if (!res?.success) {
        throw new Error(res?.message || "Failed to record sale");
      }
      toast.success("POS Direct sale completed successfully");
      setScannedProduct(null);
      setScanResult(null);
    } catch (err) {
      toast.error(err?.message || "Failed to record direct sale");
    } finally {
      setConfirming(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c0c] z-[9999] flex flex-col font-sans overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between z-50 bg-gradient-to-b from-black/80 to-transparent">
        <button
          onClick={() => navigate("/admin/direct-sales")}
          className="p-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl text-white hover:bg-white/20 transition-all flex items-center gap-2"
          title="Back to POS Direct Sales"
        >
          <ArrowLeft size={20} />
          <span className="text-xs font-semibold hidden sm:inline">Back to POS</span>
        </button>
      ...
      <div className="flex-grow relative flex items-center justify-center">
        ...
        {!scannedProduct && (
          <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
            ...
            <p className="mt-12 text-white/70 text-[11px] font-black uppercase tracking-[0.2em] animate-pulse">
          ...
          <div className="bg-white rounded-[2.3rem] p-8 flex flex-col items-center gap-6">
            ...
            <div className="w-full grid grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                ...
                <p className="text-lg font-black text-[#3E2723]">
                  ₹{Number(scannedProduct.price || 0).toLocaleString("en-IN")}
                </p>
              </div>
              <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
                ...
                <p className="text-sm font-mono font-black text-[#8D6E63] truncate">
                  {scanResult}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              disabled={confirming || !scannedProduct.available}
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-[100] flex flex-col items-center justify-center">
          ...
          <p className="text-white text-[10px] font-black uppercase tracking-[0.3em]">
            Looking up item in store catalog...
          </p>
        </div>
      )}
